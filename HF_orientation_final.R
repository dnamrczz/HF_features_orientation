...
lf <- read.csv("HF_twoshots.csv", header = T)
...
write.table(cs_lf_min_sample_30, file = "circular_statistics_min_sample_30_HF_orientation-main.csv", sep = ",")

#rose diagram for each feature with point for each feature####
rose_diagram(plunge_and_bearing(lf %>% filter(GH=='7_IV'))$bearing,          #in order to plot a different feature
             pts_on_edge = TRUE, pch = 19,                                    #change 7_IV with another feature name
             bg = "grey75", bar_col = 'grey50', main = "7_IV")

#rose diagram plunge####
rose_diagram_plunge(plunge_and_bearing(lf %>% filter(GH=='7_IV'))$plunge,    #in order to plot a different feature
                    main="Plunge7_IV", bg = 'grey25', bar_col = 'grey75')     #change 7_IV with another feature name
...
GH7_IV = lf %>%
  filter(GH == '7_IV')
GH7_IV_bearing = plunge_and_bearing(GH7_IV)$bearing
GH7_IV_bearing_df = as.data.frame(GH7_IV_bearing)
kuiper(circular(GH7_IV_bearing, type = "angles", units = "degrees"), alpha = 0.05)

###############################################
# generate the histogram of bearings by GH
# GH7_IV histogram
GH7_IV_bearing_plot = 
  ggplot(GH7_IV_bearing_df, aes(GH7_IV_bearing)) +
